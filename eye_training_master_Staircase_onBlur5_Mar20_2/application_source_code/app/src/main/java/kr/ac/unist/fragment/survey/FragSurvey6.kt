...
**/
class FragSurvey6 : Fragment() {

    // 바인딩
    lateinit var binding: FragSurvey6Binding

    // 선택한 번호
    var resultIndex: Int = -1;
    
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        this.binding = DataBindingUtil.inflate<FragSurvey6Binding>(
                inflater, R.layout.frag_survey_6, container, false)
    ...
    **/
    fun clickNext(view: View){
        if (this.resultIndex == -1) {
            ...
            var args = FragSurvey6Args.fromBundle(requireArguments())
            this.findNavController()
                .navigate(FragSurvey6Directions.actionFragSurvey6ToFragSurvey7(args.answer5 + "," + this.resultIndex.toString()))
        }
    }
