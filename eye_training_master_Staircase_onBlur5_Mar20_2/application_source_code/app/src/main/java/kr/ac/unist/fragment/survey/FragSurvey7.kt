...
**/
class FragSurvey7 : Fragment() {

    // 바인딩
    lateinit var binding: FragSurvey7Binding

    // 선택한 번호
    var resultIndex: Int = -1;
    
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        this.binding = DataBindingUtil.inflate<FragSurvey7Binding>(
                inflater, R.layout.frag_survey_7, container, false)
    ...
    **/
    fun clickNext(view: View){
        if (this.resultIndex == -1) {
            ...
            var args = FragSurvey7Args.fromBundle(requireArguments())
            this.findNavController()
                .navigate(FragSurvey7Directions.actionFragSurvey7ToFragSurvey8(args.answer6 + "," + this.resultIndex.toString()))
        }
    }
