...
import kr.ac.unist.databinding.FragSurvey5Binding
...
**/
class FragSurvey5 : Fragment() {

    // 바인딩
    lateinit var binding: FragSurvey5Binding

    // 선택한 번호
    var resultIndex: Int = -1;
    
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        this.binding = DataBindingUtil.inflate<FragSurvey5Binding>(
                inflater, R.layout.frag_survey_5, container, false)
    ...
    **/
    fun clickNext(view: View){
        if (this.resultIndex == -1) {
            ...
            var args = FragSurvey5Args.fromBundle(requireArguments())
            this.findNavController()
                .navigate(FragSurvey5Directions.actionFragSurvey5ToFragSurvey6(args.answer4 + "," + this.resultIndex.toString()))
        }
    }
