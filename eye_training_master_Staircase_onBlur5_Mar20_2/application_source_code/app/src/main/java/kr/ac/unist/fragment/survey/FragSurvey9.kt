...
**/
class FragSurvey9 : Fragment() {

    // 바인딩
    lateinit var binding: FragSurvey9Binding

    // 선택한 번호
    var resultIndex: Int = -1;
    
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        this.binding = DataBindingUtil.inflate<FragSurvey9Binding>(
                inflater, R.layout.frag_survey_9, container, false)
    ...
    **/
    fun clickNext(view: View){
        if (this.resultIndex == -1) {
            ...
            var args = FragSurvey9Args.fromBundle(requireArguments())
            this.findNavController()
                .navigate(FragSurvey9Directions.actionFragSurvey9ToFragSurvey10(args.answer8 + "," + this.resultIndex.toString()))
        }
    }
