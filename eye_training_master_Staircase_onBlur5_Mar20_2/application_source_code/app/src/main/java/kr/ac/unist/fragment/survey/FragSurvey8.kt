...
**/
class FragSurvey8 : Fragment() {

    // 바인딩
    lateinit var binding: FragSurvey8Binding

    // 선택한 번호
    var resultIndex: Int = -1;
    
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        this.binding = DataBindingUtil.inflate<FragSurvey8Binding>(
                inflater, R.layout.frag_survey_8, container, false)
    ...
    **/
    fun clickNext(view: View){
        if (this.resultIndex == -1) {
            ...
            var args = FragSurvey8Args.fromBundle(requireArguments())
            this.findNavController()
                .navigate(FragSurvey8Directions.actionFragSurvey8ToFragSurvey9(args.answer7 + "," + this.resultIndex.toString()))
        }
    }
