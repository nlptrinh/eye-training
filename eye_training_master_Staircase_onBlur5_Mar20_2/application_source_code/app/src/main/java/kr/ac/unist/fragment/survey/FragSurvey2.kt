...
**/
class FragSurvey2 : Fragment() {

    // 바인딩
    lateinit var binding: FragSurvey2Binding

    // 선택한 번호
    var resultIndex: Int = -1;
    
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        this.binding = DataBindingUtil.inflate<FragSurvey2Binding>(
                inflater, R.layout.frag_survey_2, container, false)
    ...
    fun clickNext(view: View){
        if (this.resultIndex == -1) {
            ...
            this.findNavController()
                .navigate(FragSurvey2Directions.actionFragSurvey2ToFragSurvey3(this.resultIndex.toString()))
        }
    }
